...
public class TweetParser {
    ...
    static String extractColumn(String csvLine, int csvColumn) {
        if (csvLine == null) {
            ...
            if (csvColumn >= 0 && csvColumn < csvArray.length) {
                return csvArray[csvColumn];
            } else {
                return null;
            }
        }
    ...
    static List<String> csvDataToTweets(BufferedReader br, int tweetColumn) {
        ...
        while (li.hasNext()) {
            currTweet = li.next();
            tweetCol = extractColumn(currTweet, tweetColumn);
            if (tweetCol != null) {
                tweetList.add(tweetCol);
            }
        }
        return tweetList;
        // Complete this method.
    ...
    static List<String> parseAndCleanSentence(String sentence) {
        ...
        for (int i = 0; i < wordArr.length; i++) {
            String cleaned = cleanWord(wordArr[i]);
            if (cleaned != null) {
                wordList.add(cleaned);
            }
        }
        return wordList; // Complete this method.
    }
    ...
    static List<List<String>> parseAndCleanTweet(String tweet) {
        List<List<String>> sentList = new ArrayList<>();
        String cleanedTweet = removeURLs(tweet);
        List<String> tweetList = tweetSplit(cleanedTweet);
        if (!tweetList.isEmpty()) {
            for (String tweetElem : tweetList) {
                List<String> wordList = parseAndCleanSentence(tweetElem);
                if (!wordList.isEmpty()) {
                    sentList.add(wordList);
                }
            }
        }
        return sentList; // Complete this method.
    }
    ...
    public static List<List<String>> csvDataToTrainingData(
        ...
        List<String> tweetsList = csvDataToTweets(br, tweetColumn);
        List<List<String>> dataList = new ArrayList<>();
        for (String tweet : tweetsList) {
            dataList.addAll(parseAndCleanTweet(tweet));
        }
        return dataList; // Complete this method
    }
