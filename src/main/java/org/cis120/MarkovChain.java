...
public class MarkovChain implements Iterator<String> {
    ...
    void addBigram(String first, String second) {
        if (first == null) {
            ...
            if (chain.containsKey(first)) {
                ...
                ProbabilityDistribution<String> newProb = new ProbabilityDistribution<>();
                newProb.record(second);
                chain.put(first, newProb);
            }
        }
    ...
    @Override
    public String next() {
        String copy = this.wordToReturn;
        if (this.wordToReturn == null) {
            throw new NoSuchElementException("No more words.");
        } else if (this.chain.containsKey(copy)) {
            this.wordToReturn = this.chain.get(copy).pick(ng);
        } else {
            this.wordToReturn = null;
        }
        return copy; // Complete this method.
    }
