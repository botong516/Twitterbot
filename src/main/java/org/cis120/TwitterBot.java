...
public class TwitterBot {
    ...
    public String generateTweet(int numWords) {
        ...
        String tweetCreated = "";
        if (numWords < 0) {
            throw new IllegalArgumentException("number of words in a tweet cannot be negative.");
        } else if (!mc.hasNext() || numWords == 0) {
            return "";
        } else {
            while (wordCreated < numWords) {
                if (mc.hasNext()) {
                    tweetCreated = tweetCreated + " " + mc.next();
                    wordCreated++;
                } else {
                    tweetCreated += randomPunctuation();
                    this.mc.reset();
                }
            }
        }
        return (tweetCreated + randomPunctuation()).substring(1);
    }
